  ...
  return (
    <div className="space-y-8">
      <PageHeader
        ...
        description={
          isParalegal
            ? "Assigned task work and the attorney monthly calendar. Dates you add stay highlighted until the attorney confirms them."
            : "Work items tied to your assigned matters, plus the monthly attorney calendar of important dates."
        }
      />
      ...
      <section>
        <h2 className="mb-3 text-lg font-semibold text-navy-900">
          {isParalegal ? "Assigned tasks" : "Matter tasks"}
        </h2>
        ...
        {isParalegal && (
          <div className="mb-4 flex flex-wrap gap-2">
            ...
            <Badge variant="danger">
              Overdue{" "}
              {PARALEGAL_TASKS.filter((t) => t.status === "overdue").length}
            </Badge>
            <Badge variant="warning">
              Blocked{" "}
              {PARALEGAL_TASKS.filter((t) => t.status === "blocked").length}
            </Badge>
            <Badge variant="gold">
              Waiting on attorney{" "}
              {
                PARALEGAL_TASKS.filter((t) => t.status === "waiting_on_attorney")
                  .length
              }
            </Badge>
          </div>
