...
export default function AttorneyMattersPage() {
  ...
  const matters = role === "paralegal" ? getParalegalHubMatters() : DEMO_MATTERS;

  return (
    <div>
      <PageHeader
        title={role === "paralegal" ? "Assigned Matters" : "My Matters"}
        description={
          role === "paralegal"
            ? "Assigned matters only. You cannot open, close, archive, or change fee arrangements."
            : "Assigned cases with billing arrangements for your attorney workflow."
        }
      />

      {role === "paralegal" && (
        <div className="mb-4 space-y-2">
          {PARALEGAL_ASSIGNED_MATTERS.filter((m) => m.status === "on_hold" || m.conflictStatus !== "cleared").map(
            (m) => (
              <div
                key={m.id}
              ...
              >
                ...
                <p className="mt-1">
                  ...
                  {m.status === "on_hold" ? " · Matter on hold" : ""}. Report info only — do not clear conflicts.
                </p>
              </div>
            ),
          )}
