...
import { MessageInbox } from "@/components/client-related-matters/MessageInbox";
import { Button } from "@/components/ui/Button";
import { Input } from "@/components/ui/Input";
import { Select } from "@/components/ui/Select";
import { Textarea } from "@/components/ui/Textarea";
import type { ResolvedMatter } from "@/lib/client-related-matters/data";
import { addOutgoingMessage } from "@/lib/client-related-matters/messages-store";
import { cn } from "@/lib/utils/cn";

type Step = "recipients" | "matters" | "topic" | "compose" | "sent";
...
function MessageComposer({ matters }: { matters: ResolvedMatter[] }) {
  const attachmentInputRef = useRef<HTMLInputElement>(null);

  const matterTeam = useMemo<RecipientOption[]>(() => {
    const unique = new Map<string, RecipientOption>();
    for (const matter of matters) {
      for (const member of matter.team) {
  ...
  const clientRecipients = useMemo<RecipientOption[]>(() => {
    const unique = new Map<string, RecipientOption>();
    for (const matter of matters) {
      unique.set(matter.clientId, {
  ...
  useEffect(() => {
    const allowed = new Set(matters.map((matter) => matter.id));
    setSelectedMatterIds((current) => current.filter((id) => allowed.has(id)));
  }, [matters]);

  useEffect(() => {
    const allowed = new Set(allRecipients.map((option) => option.value));
    setRecipients((current) =>
      current.map((value) => (allowed.has(value) ? value : "")),
    );
  }, [allRecipients]);
    ...
    return (
      <div className="mx-auto max-w-2xl rounded-2xl border border-gray-200 bg-white px-6 py-12 text-center shadow-sm sm:px-10">
        ...
        <p className="mx-auto mt-2 max-w-md text-sm text-muted">
          The selected clients and firm team members will receive your message
          through CounselFlow.
        </p>
        <p className="mx-auto mt-3 max-w-md text-sm text-navy-900">
          Related matter
          {relatedMatters.length === 1 ? "" : "s"}:{" "}
    ...
    <div className="mx-auto max-w-3xl">
      ...
      >
        {MESSAGE_STEPS.map((item, index) => {
          const currentIndex = MESSAGE_STEPS.indexOf(
            step as Exclude<Step, "sent">,
          );
          return (
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          <>
            <div className="mb-7 text-center">
              ...
              <p className="mt-2 text-sm text-muted">
                Choose clients, attorneys, or paralegals associated with the
                selected matters.
              </p>
            </div>
            ...
            {recipients.length < allRecipients.length && (
              <Button
                variant="ghost"
                className="mt-4"
        ...
        {step === "matters" && (
          <>
            <div className="mb-7 text-center">
              ...
              <p className="mt-2 text-sm text-muted">
                Select one or more matters. You can choose multiple matters if
                the message applies to more than one.
              </p>
            </div>
            ...
            <fieldset className="space-y-3">
              ...
              {matters.map((matter) => {
                const checked = selectedMatterIds.includes(matter.id);
                  ...
                  >
                    ...
                    <span className="min-w-0">
                      ...
                      <span className="mt-1 block text-sm text-muted">
                        {matter.clientName} — {matter.caseType}
                      </span>
                    </span>
                  </label>
                );
            ...
            <div className="mt-8 flex justify-between gap-3">
              ...
              <Button onClick={continueFromMatters}>Continue</Button>
            </div>
          </>
        )}
        ...
        {step === "topic" && (
          <>
            ...
            <div className="mt-8 flex justify-between gap-3">
              <Button variant="ghost" onClick={() => setStep("matters")}>
                <ArrowLeft className="h-4 w-4" />
                Back
              </Button>
...
export function Messaging({ matters }: { matters: ResolvedMatter[] }) {
  ...
  return (
    <div>
      <div
        className="mx-auto mb-6 flex max-w-3xl flex-wrap gap-2"
        role="tablist"
        aria-label="Messaging views"
      >
        <button
          type="button"
          role="tab"
      ...
      {view === "inbox" ? (
        <MessageInbox matters={matters} onNewMessage={startNewMessage} />
      ) : (
        <MessageComposer key={composerKey} matters={matters} />
      )}
    </div>
  );
}
